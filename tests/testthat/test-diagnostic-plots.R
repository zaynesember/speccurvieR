# Tests for the diagnostic plots: plotInfluence, plotCoefFit, plotMultiSE
...
test_that("plotInfluence returns a ggplot with included/excluded groups", {
  s <- sca_diag()
  p <- plotInfluence(s)
...
test_that("plotCoefFit returns a ggplot and validates the metric", {
  s <- sca_diag()
  expect_s3_class(plotCoefFit(s), "ggplot")
  expect_s3_class(plotCoefFit(s, metric = "adjR"), "ggplot")
  expect_error(plotCoefFit(s, metric = "AIC"), "must be one of")
})

test_that("plotMultiSE returns a ggplot with one facet per SE type", {
  p <- suppressMessages(
    plotMultiSE("Salnty", "T_degC", c("ChlorA", "O2Sat"), bottles,
                types = c("iid", "HC1", "HC3")))
  expect_s3_class(p, "ggplot")
...
test_that("plotMultiSE significance reflects the SE type", {
  p <- suppressMessages(
    plotMultiSE("Salnty", "T_degC", c("ChlorA", "O2Sat"), bottles,
                types = c("iid", "HC3")))
