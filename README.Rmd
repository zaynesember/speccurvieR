...
(`CL_FE` is clustered by the fixed-effect variable, i.e. the default `fixest::feols()` reports.) And it isn't limited to OLS–pass a `family` (and optionally a `link`), exactly as you would to `sca()`, to compare standard error types for any `glm()` family, with every error type carrying over:
